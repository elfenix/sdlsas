...
void Explosion::draw()
{
    ...
    for( i = 0; i < numPTS; i++ )
    {
        ...
        glVertex2f( GLfloat( x + scale * f_math::sin( angle ) ),
                GLfloat( y + scale * f_math::cos( angle ) ) );

        glColor4f( 1.0, 1.0, 1.0, 0.25 );
        glVertex2f( GLfloat( x + scale * f_math::sin( angle + 27 ) ),
                GLfloat( y + scale * f_math::cos( angle + 27 ) ) );
        glEnd();
    }
}
