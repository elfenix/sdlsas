...
static void sb_getpixelB1( SDL_Surface* v, SDL_Surface* d, int x, int y,
        Uint32* color )
{
    Uint32 tmp;
    Uint8 *ubuff8;
...
static void sb_getpixelB2( SDL_Surface* v, SDL_Surface* d, int x, int y,
        Uint32* color )
{
    Uint32 tmp;
    Uint8 *ubuff8;
...
static void sb_getpixelB3( SDL_Surface* v, SDL_Surface* d, int x, int y,
        Uint32* color )
{
    Uint8 *ubuff8;
    Uint32 tmp;
...
static void sb_getpixelB4( SDL_Surface* v, SDL_Surface* d, int x, int y,
        Uint32* color )
{
    Uint32* ubuff32;
...
GLuint ScreenBitmap::GL_LoadTexture( SDL_Surface *surface,
        ScreenBitmap::TexCoord& texcoord )
{
    ...
    if( !surface || !surface->format )
    {
        throw std::runtime_error(
                "Attempt to load non-existent of invalid surface" );
    }
    ...
    for( x = 0; x < (Uint32) surface->w; x++ )
    {
        for( y = 0; y < (Uint32) surface->h; y++ )
        {
            ...
            default:
                throw std::runtime_error(
                        "Unable to load texture from unsupported format" );
            }
...
void ScreenBitmap::load_image( const char* file )
{
    ...
    {
        throw std::runtime_error(
                std::string( "Couldn't load bitmap from file: " ) + file );
    }
